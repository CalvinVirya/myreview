...
function Header() {
  ...
  const mobileNavItemClass = "text-2xl font-montserrat-bold text-serpentine hover:text-ivy transition-colors";

  return (
    <Fragment>
      <header className={headerClassName}>
        <Link to="/" className="hover:scale-105 transition-transform duration-300 z-50">
        ...
        <div className="flex items-center gap-4 z-50">
          ...
          >
            {isMobileMenuOpen ? (
               <X size={28} className={isTransparent ? "text-first-frost" : "text-serpentine"} />
            ) : (
               <Menu size={28} className={isTransparent ? "text-first-frost" : "text-serpentine"} />
            )}
          </button>
        </div>

        <div className={`fixed inset-0 bg-spring/95 backdrop-blur-xl z-40 flex flex-col items-center justify-center gap-8 transition-transform duration-500 ease-in-out md:hidden ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
            <Link to="/" className={mobileNavItemClass} onClick={() => setIsMobileMenuOpen(false)}>Home</Link>
            <Link to="/business-mode" className={mobileNavItemClass} onClick={() => setIsMobileMenuOpen(false)}>Business Mode</Link>
            <Link to="/about-us" className={mobileNavItemClass} onClick={() => setIsMobileMenuOpen(false)}>About Us</Link>
            <Link to="/nearby" className={mobileNavItemClass} onClick={() => setIsMobileMenuOpen(false)}>Nearby</Link>
            <Link to="/bookmark" className={mobileNavItemClass} onClick={() => setIsMobileMenuOpen(false)}>Bookmark</Link>
            
            <div className="mt-4">
                 {storedUser ? (
                    <div className="flex flex-col items-center gap-3" onClick={() => {setIsMobileMenuOpen(false); setShowModal(true);}}>
                        <img
                        src={userImage}
                        alt="User"
                        className="w-20 h-20 rounded-full object-cover border-4 border-white shadow-xl"
                        />
                        <span className="font-montserrat-semibold text-serpentine text-xl">{userName}</span>
                    </div>
                ) : (
                    <button
